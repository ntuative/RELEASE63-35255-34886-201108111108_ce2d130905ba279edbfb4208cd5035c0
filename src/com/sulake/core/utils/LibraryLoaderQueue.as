package com.sulake.core.utils
{
   ...
   public class LibraryLoaderQueue extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_2060:int = 4;
      ...
      {
         return this.var_496.length + this.var_314.length;
      }
      ...
      private function next() : void
      {
         ...
         if(!disposed)
         {
            while(this.var_314.length < const_2060 && this.var_496.length > 0)
            {
               _loc1_ = this.var_496.shift();
               this.var_314.push(_loc1_);
               _loc1_.method_4();
            }
         }
      }
      ...
      private function removeLoader(param1:LibraryLoader) : void
      {
         ...
         try
         {
            index = this.var_496.indexOf(loader);
            if(index > -1)
            {
               this.var_496.splice(index,1);
            }
            index = this.var_314.indexOf(loader);
            if(index > -1)
            {
               this.var_314.splice(index,1);
            }
         }
         catch(e:Error)
         {
      ...
      private function isUrlInQueue(param1:String, param2:Boolean = true) : Boolean
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_496)
            {
               if(param2)
               {
      ...
      public function findLibraryLoaderByURL(param1:String, param2:Boolean = true) : LibraryLoader
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_314)
            {
               if(param2)
               {
                  if(_loc3_.url.indexOf(param1) == 0)
